...
function choiceDay(obj,index,condition){
   ...
   sideInfo.innerHTML = "<div id ='chDate'>"+replaceDate(obj.innerText)+"</div>";
	...
	let sendJsonData = [];
	sendJsonData.push({date:chDate.innerText});
	let clientData = JSON.stringify(sendJsonData);
	postAjax('schedule/getSchedule',clientData,"getScheduleList");
	
}

function getScheduleList(jsonData){
	alert(jsonData);
	...
	for(i=0; i < jsonData.length; i++){
		text += jsonData[i].title + " " + jsonData[i].location + " " + jsonData[i].contents + "<br>";
	}
	
	getSchedule.innerHTML = text;
}
